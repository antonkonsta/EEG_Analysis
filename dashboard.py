...
class FilterConfig:
    ...
    def enabled(self) -> bool:
        """Backward compatibility - returns True if any filter is enabled"""
        return self.lowpass_enabled or self.notch_enabled
    ...
    def __str__(self):
        ...
        if self.lowpass_enabled:
            parts.append(f"Low-pass: {self.lowpass_cutoff}Hz")
        if self.notch_enabled:
            parts.append(f"Notch: {self.notch_freq}Hz (Q={self.notch_q})")
